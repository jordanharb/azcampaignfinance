TEMP_FUNC <- function(.filepath,.sheet,.stcell=c(1,1)) {
  #####
  ...
  returndat <- TEMP_Dat %>% left_join(
    ...
    row != .stcell[1] & !es_is_blank #Original is_blank doesn't handle the auto-formatting decisions of Cognos very well
  ) %>% transmute(
    sheet,
    row,
    colname,
